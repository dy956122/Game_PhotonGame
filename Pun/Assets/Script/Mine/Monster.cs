...
public class Monster : MonoBehaviour,IPunObservable
{
    // 這是關於 怪物動作 的腳本
    ...
    public void Track()
    {
        ...
        monster.transform.LookAt(target);   // 讓怪物面向中心處
    }
    ...
    public void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.IsWriting)
        {
            stream.SendNext(monsterSpeed);
        }
        else
        {
            monsterSpeed = (float)stream.ReceiveNext();
        }
    }
}
